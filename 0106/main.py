...
def main() :
    ...
    dfForAccuracy = pd.DataFrame(index=list(range(epochs)),
                                 columns=["Epoch", "Train Loss", 'Train Accuracy', 'Valid Loss', 'Valid Accuracy'])
    if os.path.exists(save_path):
        best_val_acc = max(pd.read_csv("./modelAccuracy.csv")["Valid Accuracy"].tolist())
    ...
    for epoch in range(epochs):
        ...
        dfForAccuracy.loc[epoch, 'Train Loss'] = round(loss.item(), 3)
        dfForAccuracy.loc[epoch, 'Train Accuracy'] = round(train_accuracy, 3)
        dfForAccuracy.loc[epoch, 'Valid Loss'] = round(runing_loss / train_steps, 3)
        dfForAccuracy.loc[epoch, 'Valid Accuracy'] = round(val_accuracy, 3)
